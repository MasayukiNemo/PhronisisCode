import AppKit
import Carbon
import CoreGraphics

/// 修飾キー + キーコード の組み合わせ。UserDefaults保存用にCodable。
struct KeyCombo: Codable, Equatable, Hashable, Sendable {
    var keyCode: UInt16
    var modifierFlags: UInt64  // CGEventFlags rawValue
    var displayName: String?

    init(keyCode: UInt16, modifierFlags: UInt64, displayName: String? = nil) {
        self.keyCode = keyCode
        self.modifierFlags = modifierFlags
        self.displayName = displayName
    }

    /// 人間可読な表示 (例: ⌘C, ⇧⌘T)
    var readable: String {
        if let d = displayName, !d.isEmpty { return d }
        var parts: [String] = []
        let flags = CGEventFlags(rawValue: modifierFlags)
        if flags.contains(.maskCommand) { parts.append("⌘") }
        if flags.contains(.maskShift) { parts.append("⇧") }
        if flags.contains(.maskAlternate) { parts.append("⌥") }
        if flags.contains(.maskControl) { parts.append("⌃") }
        // keyCode -> 文字
        let keyName = KeyCombo.keyCodeToString(keyCode)
        parts.append(keyName)
        return parts.joined()
    }

    static func keyCodeToString(_ code: UInt16) -> String {
        // 主なキーだけマップ、なければ "Key\(code)"
        let map: [UInt16: String] = [
            0: "A", 1: "S", 2: "D", 3: "F", 4: "H", 5: "G", 6: "Z", 7: "X", 8: "C", 9: "V",
            11: "B", 12: "Q", 13: "W", 14: "E", 15: "R", 16: "Y", 17: "T",
            18: "1", 19: "2", 20: "3", 21: "4", 22: "6", 23: "5", 24: "=", 25: "9", 26: "7",
            27: "-", 28: "8", 29: "0", 30: "]", 31: "O", 32: "U", 33: "[", 34: "I", 35: "P",
            36: "↩", 37: "L", 38: "J", 39: "'", 40: "K", 41: ";", 42: "\\", 43: ",", 44: "/", 45: "N",
            46: "M", 47: ".", 48: "⇥", 49: "Space", 51: "⌤", 53: "⎋",
            96: "F5", 97: "F6", 98: "F7", 99: "F3", 100: "F8", 101: "F9", 103: "F11", 105: "F13", 106: "F16", 107: "F14", 109: "F10", 111: "F12", 113: "F15",
            118: "F4", 119: "F2", 120: "F1",
            122: "F1", 123: "←", 124: "→", 125: "↓", 126: "↑",
        ]
        if let s = map[code] { return s }
        return "Key\(code)"
    }

    /// NSEvent.ModifierFlags -> CGEventFlags 変換
    static func cgFlags(from nsFlags: NSEvent.ModifierFlags) -> CGEventFlags {
        var cg: CGEventFlags = []
        if nsFlags.contains(.command) { cg.insert(.maskCommand) }
        if nsFlags.contains(.shift) { cg.insert(.maskShift) }
        if nsFlags.contains(.option) { cg.insert(.maskAlternate) }
        if nsFlags.contains(.control) { cg.insert(.maskControl) }
        return cg
    }
}
